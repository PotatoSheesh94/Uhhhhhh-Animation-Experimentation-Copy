...
-- ==============================================================
--  ADMIN / BAN SYSTEM
--  Add your Roblox UserId(s) to the Owners table below to make
--  yourself an admin/owner of this script.
-- ==============================================================
local AdminSystem = {}
...
local _BanDurationUnits = {
        ...
        perm = -1,      permanent = -1, forever = -1,
}

AdminSystem.IsOwner = function(userId)
        for _, id in AdminSystem.Owners do
                if id == userId then return true end
        end
        return false
end

AdminSystem.IsBanned = function(userId)
        local bans = SaveData.AdminBans or {}
        local entry = bans[tostring(userId)]
        if not entry then return false end
        if entry == -1 then return true end
        if os.time() < entry then return true end
        bans[tostring(userId)] = nil
        SaveData.AdminBans = bans
        return false
end

AdminSystem.GetBanExpiry = function(userId)
        local bans = SaveData.AdminBans or {}
        return bans[tostring(userId)]
end

AdminSystem.BanPlayer = function(userId, durationSeconds)
        SaveData.AdminBans = SaveData.AdminBans or {}
        if durationSeconds == -1 then
                SaveData.AdminBans[tostring(userId)] = -1
        else
                SaveData.AdminBans[tostring(userId)] = os.time() + durationSeconds
        end
end

AdminSystem.UnbanPlayer = function(userId)
        SaveData.AdminBans = SaveData.AdminBans or {}
        SaveData.AdminBans[tostring(userId)] = nil
end

AdminSystem.FormatTimeLeft = function(expiry)
        if expiry == -1 then return "permanently" end
        local secs = math.max(0, expiry - os.time())
        ...
        for _, u in units do
                if secs >= u[1] then
                        ...
                        return n .. " " .. u[2] .. (n ~= 1 and "s" or "")
                end
        end
        return secs .. " seconds"
end

AdminSystem.ParseDuration = function(amountStr, unitStr)
        local amount = tonumber(amountStr)
        if not amount then return nil end
        unitStr = (unitStr or ""):lower()
        local multiplier = _BanDurationUnits[unitStr]
...
AdminSystem.FreezePlayer = function(targetPlayer)
        ...
        local frozen = {}
        AdminSystem._FrozenPlayers[targetPlayer] = frozen
        for _, part in char:GetDescendants() do
                if part:IsA("BasePart") then
                        local was = part.Anchored
                        part.Anchored = true
                        table.insert(frozen, {part, was})
                end
        end
end

AdminSystem.UnfreezePlayer = function(targetPlayer)
        local frozen = AdminSystem._FrozenPlayers[targetPlayer]
        if not frozen then return end
        for _, entry in frozen do
                pcall(function() entry[1].Anchored = entry[2] end)
        end
        AdminSystem._FrozenPlayers[targetPlayer] = nil
end

-- On script load: block banned players from running the script
do
        if AdminSystem.IsBanned(Player.UserId) then
                local expiry = AdminSystem.GetBanExpiry(Player.UserId)
                local timeStr = AdminSystem.FormatTimeLeft(expiry)
                local msg = expiry == -1
                        and "You are permanently banned from using Uhhhhhh."
                        or  "You are banned from Uhhhhhh for " .. timeStr .. "."
                Util.Notify(msg)
                _G.UhhhhhhLoaded = nil
                error("[Uhhhhhh] " .. msg)
        end
...
local UIMainWindow, WindowContent do
        ...
        _RefreshWindowScale = function()
                ...
                _UIWindowScaler.Scale = autoScale
        end
        _RefreshWindowScale()
...
do
        ...
        local function ShowDangerZoneDialog(onConfirmed)
                ...
                lbl.TextXAlignment = Enum.TextXAlignment.Center
                lbl.ZIndex = 9999
                lbl.Text = "Are you sure you wanna do this?\n<font color=\"#FF4444\">This action cannot be undone.</font>"
                RegisterTextLabel(lbl)
                local function MakeBtn(txt)
                        local btn = Util.Instance("TextButton", box)
                        btn.Size = UDim2.new(1, 0, 0, 30)
